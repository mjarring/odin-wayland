...
get_procedures_text :: proc(
  ...
  for procedure in procedures {
    ...
    fmt.sbprintf(
      &sb,
      `"%v", "%v", raw_data(%v_types)[%v:]`,
      procedure.name,
      get_procedure_signature(procedure),
      protocol_name,
...
parse_file :: proc(filename: string) -> Protocol {
  ...
  for interface_id in iterate_child(doc, 0, "interface") {
    ...
    interface.version, found = find_attr(
      doc,
      interface_id,
      "version",
    ); assert(found)
...
`,
    ...
    for request in interface.requests {
      ...
      opcode_name := fmt.aprintf(
        "%v_%v",
        strings.to_upper(interface.name),
        strings.to_upper(request.name),
      )
      fmt.sbprintfln(&sb, "%v :: %v", opcode_name, opcode)

      fmt.sbprintf(
        &sb,
        `%[0]v_%[1]v :: proc "contextless" (%[0]v_: ^%[0]v`,
        interface.name,
        request.name,
      )
      ...
      fmt.sbprintf(
        &sb,
        "proxy_marshal_flags(cast(^proxy)%v_, %v",
        interface.name,
        opcode_name,
      )
...
`,
    ...
    if len(interface.events) > 0 {
      ...
      fmt.sbprintfln(
        &sb,
        `%v_add_listener :: proc "contextless" (%[0]v_: ^%[0]v, listener: ^%[0]v_listener, data: rawptr) {{`,
        interface.name,
      )
      fmt.sbprintfln(
        &sb,
        "\tproxy_add_listener(cast(^proxy)%v_, cast(^generic_c_call)listener,data)",
        interface.name,
      )
      fmt.sbprintln(&sb, "}")
    ...
    if len(interface.requests) > 0 {
      ...
      fmt.sbprintln(
        &sb,
        get_procedures_text(interface.requests, requests_name, protocol.name),
      )
    }
    ...
    if len(interface.events) > 0 {
      ...
      fmt.sbprintln(
        &sb,
        get_procedures_text(interface.events, events_name, protocol.name),
      )
    }
  ...
  for interface in protocol.interfaces {
    ...
    fmt.sbprintfln(
      ...
      `%v_interface.name = "%v"`,
      interface.name,
      interface.unstripped_name,
    )
    fmt.sbprintfln(
      &sb,
      "\t%v_interface.version = %v",
      interface.name,
      interface.version,
    )
    fmt.sbprintfln(
...
foreign wl_lib {
   ...
   proxy_set_queue                           :: proc(p: ^proxy, queue: ^event_queue) ---
    ...
    if wayland_dir == "" {
      ...
      wayland_abs :=
        filepath.abs(wayland_dir, context.temp_allocator) or_else wayland_dir
...
main :: proc() {
  ...
  package_name :=
    options.package_name if options.package_name != "" else protocol.name
  code := generate_code(
    protocol,
    package_name,
    options.output,
