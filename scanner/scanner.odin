    ...
    for interface in interfaces {
        for &request in interface.requests {
            for other in interfaces {
                if other.name ==
                   fmt.aprintf("%v_%v", interface.name, request.name) {
...
`,
        ...
        for request in interface.requests {
            ...
            fmt.sbprint(&sb, ") ")
            return_type :=
                get_argument_text(request.ret.?) if has_ret else "rawptr"
...
foreign wl_lib {
   ...
   proxy_set_queue                           :: proc(p: ^proxy, queue: ^event_queue) ---
        ...
        if wayland_dir == "" {
            ...
            rel_import :=
                filepath.rel(output_dir_abs, wayland_abs) or_else ".."
